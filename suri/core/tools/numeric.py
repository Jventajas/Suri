...
@tool
def numeric_eval(expression: str, digits: int = DEFAULT_DIGITS, variable_values: dict[str, str] | None = None) -> str:
    """Evaluate an expression to a high-precision decimal — for exact formulas use `sympy_eval`.

    `expression`: a SymPy expression string, e.g. "exp(pi*sqrt(163))".
